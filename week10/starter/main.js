...
function renderProducts() {
  products.forEach((product) => {
      ...
      <div class="card h-100 d-flex flex-column rounded-0">
        <a href="#" data-bs-toggle="modal" data-bs-target="#productModal" class="image-button">
          <img src="../../assets/images/${
            product.image
          }" class="img-fluid mb-3" alt="${product.title}">
        </a>
        <div class="card-body d-flex flex-column">
          <h3 class="card-title text-center mb-3">${product.title}</h3>
          <label for="fabric-${
            product.id
          }" class="form-label mb-0 me-2">Fabric:</label>
          <select 
          ...
          <div class="mt-auto d-flex justify-content-between align-items-center">
            <p class="mb-0" id="price-${
              product.id
            }"><strong>$${firstFabricPrice}</strong></p>
    ...
    fabricSelect.addEventListener("change", function (event) {
      // Retrieve the selected fabric option from the dropdown and update the custom data attribute on the add-to-cart-button
...
productsContainer.addEventListener("click", function (event) {
  ...
  if (event.target.classList.contains("add-to-cart-button")) {
    ...
    /** Same code as the direct binding approach */
    //     // Add the cart item to the cart array
    cart.push(cartItem);
